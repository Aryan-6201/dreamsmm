...
function Feature({
  ...
  return (
    <div className="group relative overflow-hidden rounded-[22px] border border-white/80 bg-white/65 p-4 shadow-[0_10px_35px_rgba(76,29,149,0.06)] backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:bg-white/85 hover:shadow-[0_18px_45px_rgba(76,29,149,0.12)] sm:p-5">
      ...
      <div className="relative mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-500/15 to-fuchsia-500/15 text-xl shadow-inner ring-1 ring-violet-200/60 transition-transform duration-300 group-hover:scale-110">
        {icon === "fast" && (
          <Zap className="h-7 w-7 text-violet-600" strokeWidth={2.5} />
        )}

        {icon === "services" && (
          <Sparkles className="h-7 w-7 text-violet-600" strokeWidth={2.3} />
        )}

        {icon === "secure" && (
          <ShieldCheck className="h-7 w-7 text-violet-600" strokeWidth={2.3} />
        )}

        {icon === "support" && (
          <Headphones className="h-7 w-7 text-violet-600" strokeWidth={2.3} />
        )}
      </div>
