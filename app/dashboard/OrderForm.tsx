...
.category-icon-live {
  ...
  overflow: hidden;
  isolation: isolate;
  background: linear-gradient(145deg, #24262d, #0b0d12 55%, #020306) !important;
...
.category-icon-live::after {
  ...
  z-index: 5;
  pointer-events: none;
  animation: categoryShine 2.2s ease-in-out infinite;
}

.category-icon-live > svg {
  position: relative;
  z-index: 4;
                ...
                {filteredCategories.map(
                  (item) => {
                    ...
                    return (
                      ...
                      >
                        <span
  className="category-icon-live flex h-7 w-7 shrink-0 items-center justify-center rounded-md shadow-sm"
>
  {(() => {
    const platform = getCategoryPlatform(
      item,
    ...
    <span className="text-xl font-black tracking-tight text-slate-900">
      ...
      {success && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3">
          <p className="text-xs font-bold text-emerald-700">âœ“ {success}
          </p>
        </div>
      )}
